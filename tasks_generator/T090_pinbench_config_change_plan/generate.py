...
def render_local_service(task_id: str) -> str:
	return textwrap.dedent(
		f'''
		"""Task-local Config mock service for {task_id}."""
		...
		app = FastAPI(title="Task Local Config API")

		from mock_services._base import add_error_injection
		add_error_injection(app)

		FIXTURES_PATH = Path(__file__).resolve().parent / "fixtures" / "config" / "integrations.json"
		...
		def _load_fixtures() -> None:
			...
			with open(FIXTURES_PATH, encoding="utf-8") as handle:
				_integrations = json.load(handle)
...
def build_task_yaml(task_id: str, prompt_text: str, scenario: Scenario) -> dict[str, Any]:
	return {
		...
		"services": [
			{
				"name": "config",
				"command": f"python tasks/{task_id}/local_config_server.py",
				"port": 9111,
...
def build_variant(
	...
	(task_dir / "local_config_server.py").write_text(
